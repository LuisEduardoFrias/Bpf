...
export default function Form<T>(props: IFormsProp<T>): ReactElement {
 ...
 const formComponentsWithOnChange = formComponents.map((child: ReactElement,
 index: number) => addOnChangeToFormComponents(child, index, setState, setChildRequired));
 ...
 return (
  <>
   ...
   <form onSubmit={(event)=> Submit(event, childRequired, state, setShowLoader, setNotif, props.onSubmit)}
    className={props.className}>{formComponentsWithOnChange}</form>
  </>
 );
}
...
async function Submit<T>(
 ...
 onSubmit: (
  ...
  if(!thereAreRequited) {
   if(await onSubmit(state, setShowLoader, setNotif)) { 
    clear();
   }
  }
  else {
...
function requiredAlertStyles(child: ReactElement, setChildRequired: (callback: (value: RequireAlerts[]) => requiredAlertStyles[]) => void ) : ReactElement {
 if (child) {
  
  setChildRequired(prev =>
  {
   if(prev !== undefined)
    return {key: child.props.for ,element: child};
  });
...
function clear() {
 ...
 const formElements = Array.from(document.querySelectorAll("input, select, textarea")) as HTMLElement[];
...
function getInputValue(event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>): string | number {
...
function handleChange(e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>, setState: (callback: (value: object[]) => object[]) => void) : void {
  ...
  if (e.target.type === "file" && props.dataFile) {
   ...
   if (file) {
    ...
    reader.onloadend = async () => {
     setState(prev => { return {...prev, [name]: reader.result as string }; });
    };
...
//alert("change: " + e.target.name)
   ...
   setState(prev => { return { ...prev, [e.target?.name]: value }}); }
  }
  
 }
};
  
function handleChange2 (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>, change: (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => void, setState: (callback: (value: object[]) => object[]) => void ) {
  if(change) {Reflect.apply(change, null,[e]);}
  handleChange(e, setState)
};
  
function addOnChangeToFormComponents(child: ReactElement, index: number, setState: (callback: (value: object[]) => object[]) => void, setChildRequired: (callback: (value: RequireAlerts[]) => requiredAlertStyles[]) => void ): ReactElement {
 ...
 if(React.isValidElement(child)) {
  ...
  if (cloneChild.type === 'input' && (type === "text" || type === "undefined") && cloneChild.props.onChange === undefined) { 
   return React.cloneElement(cloneChild, { 
    onChange: (e:any)=>handleChange2(e,change, setState), key: index, });
  }
  ...
  if (type === "select" && cloneChild.props.onChange === undefined) {
   return React.cloneElement(cloneChild, {  onChange: (e:any)=>handleChange2(e,change, setState), key: index,});
  }

  const count = React.Children.count(cloneChild.props.children);
  
  if (count > 0) {
   const childrenWithOnChange = React.Children.map(cloneChild.props.children, (child: ReactElement, index: number) => addOnChangeToFormComponents(child,
    index, setState, setChildRequired));
